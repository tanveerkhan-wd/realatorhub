                ...
                <div class="chart_section">
                    <form id="search-form">
                        <div class="row">
                            ...
                            <div class="col-md-3 col-sm-6">
                                <div class="form-group "> 
                                    <select name="property_purpose" class="form-control select2 dropdown_control" required="" id='customer_dropdown'>
                                        @foreach($customer_year_list as $key=>$value)
                                        <option value="{{$value->year}}" @if($value->year==date('Y')) selected="selected" @endif>{{$value->year}}</option>
                                        @endforeach
                                    </select>
                                </div>
...
$(document).on('change', '#leads_dropdown', function() {
    ...
    $.ajax({
        type: "POST",
        dataType: 'json',
        url: '{{route("agency.leads.chart.data")}}',
        data: {year: year},
        headers: {
...
var customerdata = <?php echo $customer_year_count ?>;

var customerchart = Highcharts.chart('customer', {
    chart: {
        type: 'column'
    },
    title: {
...
$(document).on('change', '#customer_dropdown', function() {
    $('.loader-outer-container').css('display', 'table');
    var year = $(this).val();
    $.ajax({
        type: "POST",
        dataType: 'json',
        url: '{{route("agency.customer.chart.data")}}',
        data: {year: year},
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(data) {
            customerchart.series[0].setData(data);
            $('.loader-outer-container').css('display', 'none');
        }
    });
});
