		...
		<div class="col-lg-12">	
			...
			<div class="tab-content theme_tab_content" id="myTabContent">								  
		  		<div class="tab-pane fade @if($_REQUEST['data'] == 'editProfile') active in show @endif" id="editProfile" role="tabpanel" aria-labelledby="editProfile-tab">
		  			<form id="sign_up_form" method="POST" action="{{ url('agency/editMyProfilePost') }}" enctype="multipart/form-data">
							{{ @csrf_field() }}	
						<div class="row">
		                    <div class="col-md-3"></div>
	                      	<div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="email"  name="lead_email" class="form-control" id="lead_email" placeholder="Enter Lead Email" value="@if(isset($data['lead_email'])){{$data['lead_email']}} @endif">
                                    <small>The Email on which you will get Lead or Contact Form details from Property Page.</small> 
                                    <div id="email_validate"></div>  
                                </div>
                                <div class="form-group">
                                    <label>Phone*</label>
		  		...
		  		<div class="tab-pane fade @if($_REQUEST['data'] == 'changePassword') active show  @endif in" id="changePassword" role="tabpanel" aria-labelledby="changePassword-tab">
		  			<form method="POST" action="{{ url('agency/changePassword') }}" enctype="multipart/form-data" id="change-password-form" name="change-password-form">
					{{ @csrf_field() }}	
		  				<div class="row">
	                    	<div class="col-md-3"></div>
	                      	<div class="col-md-6">
  ...
  if(formStatus==true){
    ...
    swal({
            ...
            if (isConfirm.value == true) {                
                $.ajax({
                    type: "POST",
                    url: base_url_route+'/agency/changeEmail',
                    data:{email:email},
                    headers: {
...
$(document).on('click','#otpVerifyModelButton',function(e){
  ...
  if(formStatus==true){
        ...
        $.ajax({
                type: "POST",
                url: base_url_route+'/agency/changeEmailVeriy',
...
$(document).on('click','.deactive_account',function(){
    var is_plane_active=<?= $is_plane_active?>;
    if(is_plane_active>0){
        swal({
            text: "Please cancel your subscription before deactivating your account.",
            type: 'info',
        });
    }else{
        swal({
