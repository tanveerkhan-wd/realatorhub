        ...
        <div class="col-lg-12 mt-4">	
			...
			<div class="tab-content theme_tab_content" id="myTabContent">								  
		  		<div class="tab-pane fade @if($_REQUEST['data'] == 'editProfile') active in show @endif" id="editProfile" role="tabpanel" aria-labelledby="editProfile-tab">
		  			<form id="sign_up_form" method="POST" action="{{url('/'.$slug.'/editMyProfile')}}" enctype="multipart/form-data">
							{{ @csrf_field() }}	
						<div class="row">
	                      	...
	                      	<div class="col-md-6">
								<div class="form-row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="hidden" name="id" class="form-control" value="@if(isset($customer_data->id)){{$customer_data->id}} @endif">
                                            <label for="first_name">First Name</label>
                                            <input type="text"  pattern="\w+" name="first_name" class="form-control"
                                                   id="first_name" aria-describedby="emailHelp" placeholder="Enter First Name" value="@if(isset($customer_data->first_name)){{$customer_data->first_name}}@endif">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text"  pattern="\w+" name="last_name" class="form-control"
                                                   id="last_name" aria-describedby="emailHelp" placeholder="Enter Last Name" value="@if(isset($customer_data->last_name)){{$customer_data->last_name}}@endif">
                                            <span class="form-control-feedback"></span>
                                ...
                                <div class="form-group">
                                    <label for="email" class="edit_icon" id="changeEmailButton">Email ID <i class="far fa-edit"></i></label>
                                        <input type="email"  name="old_email" class="form-control" id="email" placeholder="Enter Email Id" value="@if(isset($customer_data->email)){{$customer_data->email}} @endif" readonly="">
                                        ...
                                        <div class="col-4">
                                            <select class="form-control dropdown_control country_code" name="country_code" id="country_code">
                                                ...
                                                <?php $phone_code="+".$val->calling_code; ?>
                                                    <option value="+{{$val->calling_code}}"  data-image="{{ url('public/uploads/country_images/'.$val->flag) }}" @if(!empty($customer_data->phone_code) && $customer_data->phone_code==$phone_code) selected @endif>{{"+".$val->calling_code." ".$val->name}}</option>
                                                @endforeach
                                            </select>
                                            <div id="country_code_validate"></div>
                                        </div>
                                        <div class="col-8">
                                            <input  type="text"  name="mobile_number" class="form-control" id="mobile_number" aria-describedby="emailHelp" placeholder="Enter Number" value="@if(isset($customer_data->phone)){{$customer_data->phone}}@endif">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="upload-file-group">
                                        <div class="choose_imd_box text-center">
                                            <?php if(isset($customer_data->profile_img) && !empty($customer_data->profile_img)){?>
                                        	<img src="{{ url('public/uploads/profile_photos').'/'.$customer_data->profile_img }}" class="" id="agency_logo_image"><br>
                                            <?php }else{?> 
                                        ...
                                        <div class="form-group ">
                                            ...
                                            <select class="select2 form-control icon_control dropdown_control timezone" name="timezone" id="timezone">
                                                    ...
                                                    @foreach($timezones as $timeZone)
                                                        <option value="{{$timeZone->id}}" @if(!empty($customer_data->timezone) && $customer_data->timezone==$timeZone->id) selected @endif>{{trim($timeZone->timezone)}}</option>
                                                    @endforeach
                                                </select>
		  		...
		  		<div class="tab-pane fade @if($_REQUEST['data'] == 'changePassword') active show  @endif in" id="changePassword" role="tabpanel" aria-labelledby="changePassword-tab">
		  			<form method="POST" action="{{url('/'.$slug.'/userChangePassword')}}" enctype="multipart/form-data" id="change-password-form" name="change-password-form">
					{{ @csrf_field() }}	
		  				<div class="row">
	                    	<div class="col-md-3"></div>
	                      	<div class="col-md-6">
...
<script type="text/javascript" src="{{ url('public/js/frontend/signup/edit_signup.js') }}"></script>
