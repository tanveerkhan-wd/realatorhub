...
class SendmailController extends Controller {
    ...
    public function index() {
        return view('agency.sendmail.index');
    }
    ...
    public function getPropertyList(Request $request) {
        ...
        if (!empty($input['customer'])) {
            ...
            $all_property = PropertyModel::leftJoin('users', 'properties.agent_id', '=', 'users.id')->leftJoin('agency', 'properties.agency_id', '=', 'agency.user_id')
                    ...
                    ->where("is_delete", "=", "0")->where('agency_id', '=', Auth::user()->id)->where('properties.status','=','1')
            ...
            $all_property = PropertyModel::leftJoin('users', 'properties.agent_id', '=', 'users.id')->leftJoin('agency', 'properties.agency_id', '=', 'agency.user_id')->where("is_delete", "=", "0")->where('agency_id', '=', Auth::user()->id)->where('properties.status','=','1')
                    ->select('properties.id', 'properties.purpose', 'properties.type', 'properties.address', 'properties.is_delete', 'properties.status', 'properties.created_at', 'users.first_name', 'users.email', 'users.last_name', 'agency.slug')
                    ->orderby('properties.id', 'desc');
        }
    ...
    public function sendGeneralEmail(Request $request) {
        ...
        if (!empty($input['customer']) && !empty($input['property']) && !empty($input['note'])) {
            ...
            foreach ($property_list as $key => $property) {
                ...
                //$str.="<div style='background-color:#eee;padding:15px;border-radius:10px;margin:15px 0px;';><div style='text-align:center;margin-bottom:15px;'> <img src=" . url('public/uploads/properties_images') . '/' . $property->id . '/' . $property->main_image . " id='agency_logo_image' style='max-height:300px;max-width:100%;'></div><br>";
