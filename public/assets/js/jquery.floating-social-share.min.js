! function($, window, document, undefined) {
    ...
    function title_case(str) { return str.replace(/\w\S*/g, function(txt) { return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase() }) }

    function shorten(num) { return num >= 1e9 ? (num / 1e9).toFixed(1).replace(/\.0$/, "") + "G" : num >= 1e6 ? (num / 1e6).toFixed(1).replace(/\.0$/, "") + "M" : num >= 1e3 ? (num / 1e3).toFixed(1).replace(/\.0$/, "") + "K" : num }

    function setMobileCss(objects) { $.each(objects, function() { isMobile ? ($(this).css("width", 100 / objects.length + "%"), $(this).removeClass("with-counter-desktop")) : ($(this).removeAttr("style"), $(this).find("span.shareCount").length > 0 && $(this).addClass("with-counter-desktop")) }) }
    ...
    function updateIsMobile() { var mobile = getWidth() < 961; return void 0 !== isMobile && (isMobile = mobile), mobile }

    function appendButtons(count, $component) { count && count > 0 && ($component.append($("<span>", { class: "shareCount" }).append(shorten(count))).removeClass("without-counter"), !1 === isMobile && $component.addClass("with-counter-desktop")) }

    function issetOrZero(fn) { var value; try { value = fn() } catch (e) { value = 0 } return value }

    function setShareCount(network, url, $component, twitter_counter) {
        switch (network) {
            case "facebook":
                $.getJSON("https://graph.facebook.com/?id=" + url + "&callback=?", function(data) { appendButtons(issetOrZero(function() { return data.share.share_count }), $component) });
                break;
            case "linkedin":
                $.getJSON("https://www.linkedin.com/countserv/count/share?url=" + url + "&callback=?", function(data) { appendButtons(issetOrZero(function() { return data.count }), $component) });
                break;
            case "odnoklassniki":
                $.getJSON("https://connect.ok.ru/dk?st.cmd=extLike&ref=" + url + "&callback=?", function() {}), window.ODKL = window.ODKL || {}, window.ODKL.updateCount = function(index, count) { appendButtons(count, $component) };
                break;
            case "pinterest":
                $.getJSON("https://api.pinterest.com/v1/urls/count.json?url=" + url.replace(/\/+$/, "/") + "&callback=?", function(data) { appendButtons(issetOrZero(function() { return data.count }), $component) });
                break;
            case "reddit":
                $.getJSON("https://www.reddit.com/api/info.json?url=" + url + "&jsonp=?", function(response) { appendButtons(issetOrZero(function() { return response.data.children[0].data.score }), $component) });
                break;
            case "tumblr":
                $.getJSON("https://api.tumblr.com/v2/share/stats?url=" + url + "&callback=?", function(data) { appendButtons(issetOrZero(function() { return data.response.note_count }), $component) });
                break;
            case "twitter":
                1 == twitter_counter && $.getJSON("https://opensharecount.com/count.json?url=" + url + "&callback=?", function(data) { appendButtons(issetOrZero(function() { return data.count }), $component) });
                break;
            case "vk":
                $.getJSON("https://vk.com/share.php?act=count&index=1&url=" + url + "&callback=?", function() {}), window.VK = window.VK || {}, window.VK.Share = window.VK.Share || {}, window.VK.Share.count = function(index, count) { appendButtons(count, $component) };
                break;
            default:
                return -1
        }
    ...
    var pluginName = "floatingSocialShare",
        defaults = { place: "top-left", counter: !0, twitter_counter: !1, target: !0, buttons: ["facebook", "twitter"], title: document.title, url: window.location.href, description: $('meta[name="description"]').attr("content") || "", media: $('meta[property="og:image"]').attr("content") || "", text: { default: "share with:" }, text_title_case: !1, popup: !0, popup_width: 400, popup_height: 300, extra_offset: 15 },
